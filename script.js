...
// Variável para guardar o scanner
let html5QrcodeScanner;

window.abrirCamera = (idInputDestino) => {
    document.getElementById('leitor-camera').style.display = 'flex';

    // Configura o leitor
    html5QrcodeScanner = new Html5QrcodeScanner(
        "reader", 
        { fps: 10, qrbox: 250 } // FPS: Quadros por segundo
    );

    // O que fazer quando ler com sucesso
    function onScanSuccess(decodedText, decodedResult) {
        // Toca um bip (opcional)
        // new Audio('bip.mp3').play(); 

        // Joga o código lido no input que você escolheu
        const input = document.getElementById(idInputDestino);
        input.value = decodedText.toUpperCase();
        
        // Fecha a câmera
        fecharCamera();
        
        // Simula o ENTER para o sistema processar
        input.dispatchEvent(new Event('keyup')); // Dispara sua lógica de detectar tipo (COL/EMP)
        
        // Se quiser salvar direto:
        // if(idInputDestino === 'ret-ativo') realizarRetirada();
    }
